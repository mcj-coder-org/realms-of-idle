---
title: 'Lightning'
type: 'enchantment'
category: 'combat'
tier: 'journeyman'
summary: 'Crackles with electrical energy that chains between foes and disrupts magic'
---

# Lightning

## Lore

### Origin

Lightning enchantments were born from storm-mages' attempts to harness thunderbolts - raw, chaotic power that destroys as readily as it protects. The breakthrough came when enchanters stopped trying to control lightning and instead learned to channel it. The item becomes a conduit, inviting electricity rather than containing it. This philosophical shift transformed lightning from a dangerous novelty into a reliable combat enchantment.

The first successful Lightning blade was created during a literal thunderstorm, and superstitious enchanters maintained that tradition for decades afterward. Modern enchanters know that storm conditions aren't required, but many still prefer to work Lightning enchantments during stormy weather. There's something fitting about calling lightning while rain pounds the workshop roof.

### In the World

Lightning items are spectacular in action - arcs of electricity leaping from item to target, the sharp crack of thunder, the smell of ozone. Young warriors dream of wielding such weapons, though veterans know the drawbacks. Lightning is loud, bright, and attracts attention. Every monster for miles knows you're coming when your sword sounds like a thunderclap with each strike.

Still, certain situations demand Lightning's unique properties. Metal-armored foes conduct the shock beautifully. Water-dwelling creatures have nowhere to hide from spreading electricity. And practitioners of magic find their concentration shattered by electrical disruption. Sky-ship marines favor Lightning weapons for boarding actions, where the noise doesn't matter and enemies cluster together perfectly for chain effects. Lightning rings have found surprising utility among signal-keepers and emergency responders - a bright flash and loud crack makes an excellent distress signal.

---

## Mechanics

### Requirements

| Requirement      | Value                                                                                              |
| ---------------- | -------------------------------------------------------------------------------------------------- |
| Enchanting Skill | [Enchanting](../../skills/index.md) (Journeyman) 20                                                |
| Reagents         | [Storm essence](../../materials/reagents/index.md), [Fulgurite](../../materials/reagents/index.md) |
| Workspace        | Enchanting table, properly grounded                                                                |

### Applicable Items

| Item Category                                 | Effectiveness  | Notes                    |
| --------------------------------------------- | -------------- | ------------------------ |
| [Weapons](../../items/weapons/index.md) (all) | Full (100%)    | Bladed or blunt          |
| [Arrows/Bolts](../../items/weapons/index.md)  | Full (100%)    | Electric projectiles     |
| [Gauntlets](../../items/armor/index.md)       | High (90%)     | Shocking punches         |
| [Rings](../../items/index.md)                 | Moderate (60%) | Touch attacks, signaling |
| [Amulets](../../items/index.md)               | Low (40%)      | Electric aura            |
| [Boots](../../items/clothing/index.md)        | High (85%)     | Shocking kicks           |
| [Shields](../../items/armor/index.md)         | Moderate (70%) | Shocks attackers         |
| [Staves](../../items/weapons/index.md)        | Full (100%)    | Excellent conductor      |
| [Rods](../../items/tools/index.md)            | High (90%)     | Utility applications     |

**Special Note:** Metal items conduct Lightning enchantments 10% more effectively than non-metal items.

### Enchantment Levels

| Level | Spark Intensity | Lightning Damage | Chain Targets | Material Cost |
| ----- | --------------- | ---------------- | ------------- | ------------- |
| I     | Static          | +1d6 lightning   | 1             | 150 copper    |
| II    | Spark           | +2d6 lightning   | 2             | 700 copper    |
| III   | Arc             | +3d6 lightning   | 3             | 3000 copper   |
| IV    | Bolt            | +4d6 lightning   | 4             | 10000 copper  |
| V     | Tempest         | +5d6 lightning   | 5             | 30000 copper  |

### Special Properties

| Property         | Effect                                             |
| ---------------- | -------------------------------------------------- |
| Chain Lightning  | Damage arcs to nearby enemies                      |
| Shock            | Chance to stun or paralyze target                  |
| Spell Disruption | Interrupts enemy spellcasting                      |
| Magnetic Pull    | Slight attraction to metal objects                 |
| Thunder          | Loud crack on each strike; alerts nearby creatures |
| Static Field     | Creates area of electric charge                    |

### Material Interactions

| Material                                   | Compatibility | Special Effect                     |
| ------------------------------------------ | ------------- | ---------------------------------- |
| [Steel](../../materials/metals/index.md)   | Excellent     | Perfect conductor, +10% damage     |
| [Silver](../../materials/metals/index.md)  | Excellent     | Enhanced magical effects           |
| [Bronze](../../materials/metals/index.md)  | Good          | Standard bonuses                   |
| [Mithril](../../materials/metals/index.md) | Fair          | Lightning appears golden-blue      |
| [Wood](../../materials/wood/index.md)      | Poor          | Risk of item damage from discharge |
| [Iron](../../materials/metals/index.md)    | Excellent     | Strong conductor                   |
| [Copper](../../materials/metals/index.md)  | Excellent     | Best conductor (+15% damage)       |

### Utility Applications (Non-Combat)

| Use Case            | Effectiveness | Item Type                 |
| ------------------- | ------------- | ------------------------- |
| Emergency signaling | Excellent     | Rings, rods, weapons      |
| Metal detection     | Moderate      | Rings, rods               |
| Ignition source     | Good          | Any enchanted item        |
| Magical disruption  | Excellent     | Any enchanted item        |
| Static charge       | Moderate      | Useful for certain crafts |

### Interactions

| System      | Interaction                                             |
| ----------- | ------------------------------------------------------- |
| Combat      | Chain damage; effective vs metal armor, water creatures |
| Environment | Can ignite flammables; dangerous in water/metal areas   |
| Magic       | Disrupts spellcasting; damages magical barriers         |
| Stealth     | Thunder effect eliminates stealth possibility           |
| Weather     | Enhanced potency during storms (+20% damage)            |
| Metal       | Attracts to metal; bonus damage vs metal-armored foes   |

---

## Related Content

- **Requires:** [Enchanting](../../skills/index.md) (Journeyman), [Storm essence](../../materials/reagents/index.md), [Fulgurite](../../materials/reagents/index.md)
- **Applied To:** Weapons, gauntlets, boots, shields, rings, staves (see Applicable Items table)
- **See Also:** [Flaming](flaming.md), [Frost](frost.md), [Thundering](thundering.md)
- **Enchantment Type:** Combat (elemental damage - electric)
