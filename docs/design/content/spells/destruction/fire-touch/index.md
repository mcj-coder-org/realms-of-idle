---
title: Fire Touch
type: spell
school: destruction
element: fire
tiers: [innate]
tags: [Magic/Destruction, Combat/Melee, Creature/Innate]
summary: Melee attack deals fire damage on contact
---

# Fire Touch

## Overview

Ignite targets through physical contact, dealing fire damage.

| Property  | Value         |
| --------- | ------------- |
| School    | Destruction   |
| Element   | Fire          |
| Targeting | Touch (melee) |

## Effects

### Innate (Creature)

- 15-25 fire damage on hit
- Ignites flammable materials
- **Scaling**: Creature level

## Innate Users

| Creature                                                               | Notes          |
| ---------------------------------------------------------------------- | -------------- |
| [Fire Elemental](../../creatures/magical/elementals/fire-elemental.md) | Primary attack |
| Flame Sprite                                                           | Lesser damage  |

## Notes

- This spell is innate to fire-based creatures
- Cannot be learned by standard magic users
- Countered by fire resistance
- Creatures immune to their own element

## Related Content

- **Innate To:** [Fire Elemental](../../creatures/magical/elementals/fire-elemental.md), Flame Sprite
- **See Also:** [Destruction Spells](../index.md), [Fire Magic](../../systems/magic/fire-magic.md), [Creature Abilities](../../systems/creatures/abilities.md)
