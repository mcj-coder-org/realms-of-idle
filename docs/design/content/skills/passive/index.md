---
title: Passive Skills
type: index
summary: Always-active skills requiring no activation
---

# Passive Skills

Skills that provide constant benefits without requiring activation.

## Mechanics

| Aspect     | Description                      |
| ---------- | -------------------------------- |
| Activation | None required - always active    |
| Cost       | No ongoing resource drain        |
| Stacking   | Multiple passives stack normally |
| Conditions | Some passives have prerequisites |

## Passive Categories

| Category   | Description              |
| ---------- | ------------------------ |
| Attribute  | Stat bonuses             |
| Resistance | Damage/effect mitigation |
| Perception | Enhanced senses          |
| Racial     | Species-specific traits  |

## Skills

| Skill                                             | Category   | Effect                    |
| ------------------------------------------------- | ---------- | ------------------------- |
| [Arcane Sensitivity](arcane-sensitivity/index.md) | Perception | Detect magical auras      |
| [Darkvision](darkvision/index.md)                 | Perception | See in darkness           |
| [Dense Build](dense-build/index.md)               | Attribute  | Increased weight/strength |
| [Elven Grace](elven-grace/index.md)               | Racial     | Improved finesse          |
| [Magic Resistance](magic-resistance/index.md)     | Resistance | Reduce magical damage     |

## Related Documentation

- [Toggle Skills](../toggle/index.md) - Sustained on/off abilities
- [Common Skills](../common/index.md) - Universally available skills
