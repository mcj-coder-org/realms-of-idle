---
title: Market Sense
type: tiered
category: tiered
summary: Know prices and opportunities
tags: [Trade, Economy]
---

# Market Sense

## Overview

Know prices and opportunities.

| Property | Value                       |
| -------- | --------------------------- |
| Pool     | Trade                       |
| Category | Tiered                      |
| Classes  | Trader, Merchant, Caravaner |

## Effects

### Lesser

- Know local prices
- Sense good deals (+15%)
- **Scaling**: +1% per class level

### Greater

- Know regional prices
- Predict price changes
- **Scaling**: +0.75% per class level

### Enhanced

- Know inter-regional prices
- Manipulate local markets
- **Scaling**: +0.5% per class level

## Acquisition

**Trigger Actions**: Trade frequently, track prices, exploit opportunities
**Classes**: Trader, Merchant, Caravaner

## Related Content

- **Strong Synergy:** [Trader](../../classes/economic/trader/), [Merchant](../../classes/economic/merchant/), [Caravaner](../../classes/economic/caravaner/)
- **See Also:** [Silver Tongue](silver-tongue.md), [Trading System](../../systems/economy/trading.md), [Economic Skills](../index.md)
