---
title: Beekeeper's Bond
type: mechanic-unlock
tier-type: Mechanic Unlock
summary: Special connection with bees
tags: [Agriculture/Livestock, Nature/Insects]
---

# Beekeeper's Bond

## Overview

**Type**: Mechanic Unlock
**Pool**: Agriculture
**Description**: Special connection with bees enabling safe interaction and enhanced yields.

## Synergy Classes

Classes with strong synergies: Beekeeper

**Synergy Benefits**: 2x XP acquisition, enhanced effectiveness, reduced costs (scales with class level)

## Mechanics

| Effect        | Description                                     |
| ------------- | ----------------------------------------------- |
| Unlock        | Bees never sting you, +50% honey/wax yield      |
| Communication | Bees warn of dangers, scout local flowers       |
| Defense       | Can direct swarm against threats (once per day) |

**No scaling—binary unlock**

## XP Sources

- Work with bees: 5-15 XP per session
- Harvest honey/wax: 10-20 XP
- Guided training: 10-100 XP/session
- Unguided training: 5-50 XP/session

## Level-Up Availability

- Strong synergy: ~500 XP (~50 actions)
- Moderate synergy: ~1000 XP
- No synergy: ~1500 XP

## Synergy Examples

### Strong (2x XP)

**Beekeeper Level 15** - Bee specialist

- +70% yield
- Enhanced communication
- 2x XP from beekeeping

### Moderate (1.5x XP)

**Farmer Level 10** - Animal handler

- +55% yield

### None (1x XP)

**Warrior** - No connection

- No bonuses

## Related Content

- **Strong Synergy:** [Beekeeper](../../classes/gatherer/beekeeper/)
- **See Also:** [Animal Husbandry](../tiered/animal-husbandry/index.md), [Agriculture System](../../systems/gathering/agriculture/index.md)
