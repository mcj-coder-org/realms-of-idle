---
title: Season Sense
type: mechanic-unlock
tier-type: Mechanic Unlock
summary: Perfect agricultural timing
tags: [Agriculture, Nature]
---

# Season Sense

## Overview

**Type**: Mechanic Unlock
**Pool**: Agriculture
**Description**: Perfect agricultural timing with weather prediction and crop adaptation.

## Synergy Classes

Classes with strong synergies: Farmer, Rancher

**Synergy Benefits**: 2x XP acquisition, enhanced effectiveness, reduced costs (scales with class level)

## Mechanics

| Effect     | Description                                         |
| ---------- | --------------------------------------------------- |
| Unlock     | Know optimal planting/harvesting times for any crop |
| Prediction | Predict weather effects on crops 1 week ahead       |
| Adaptation | Know which crops suit current conditions            |

**No scaling—binary unlock**

## XP Sources

- Farm through multiple seasons: 10-20 XP per season
- Adapt to conditions: 5-15 XP
- Guided training: 10-100 XP/session
- Unguided training: 5-50 XP/session

## Level-Up Availability

- Strong synergy: ~500 XP (~50 actions)
- Moderate synergy: ~1000 XP
- No synergy: ~1500 XP

## Related Content

- **Strong Synergy:** [Farmer](../../classes/gatherer/farmer/), [Rancher](../../classes/rancher/)
- **See Also:** [Skills Index](../index.md)

## Synergy Examples

### Strong (2x XP)

**Farmer Level 15** - Agricultural timing specialist

- Extended predictions (2 weeks)
- 2x XP from seasonal farming

### Moderate (1.5x XP)

**Rancher Level 10** - Seasonal manager

- Better adaptation

### None (1x XP)

**Thief** - No connection

- No bonuses
