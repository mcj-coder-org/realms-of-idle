---
title: Armor Fitting
type: mechanic-unlock
category: Smithing Pool
skill_type: Mechanic Unlock
synergy_classes: Armorer
tags: [Crafting/Smithing/Armor]
---

# Armor Fitting

**Type:** Mechanic Unlock
**Pool:** Smithing
**Strong Synergy Classes:** Armorer

## Effect

| Aspect     | Description                                    |
| ---------- | ---------------------------------------------- |
| Unlock     | Can create armor fitted to specific individual |
| Benefit    | Fitted armor: -50% penalties, +15% protection  |
| Limitation | Fitted armor only works for intended wearer    |

## Scaling

- **No scaling**—binary unlock

## Trigger Actions

- Create armor
- Study armor fitting

## Notes

- Custom-fitted armor provides significant advantages
- Armor loses benefits if worn by anyone else
- Represents personalized craftsmanship

## Related Content

- **Strong Synergy:** [Armorer](../../classes/crafter/armorer/)
- **See Also:** [Armor Crafting](../../systems/crafting/armor-crafting/index.md), [Armor Expert](../tiered/armor-expert/index.md)
