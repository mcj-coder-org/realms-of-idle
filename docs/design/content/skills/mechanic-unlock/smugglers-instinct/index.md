---
title: Smuggler's Instinct
type: mechanic-unlock
tier-type: Mechanic Unlock
summary: Innate sense for hiding contraband
tags: [Stealth/Infiltration]
---

# Smuggler's Instinct

## Overview

**Type**: Mechanic Unlock
**Pool**: Stealth
**Description**: Innate sense for hiding contraband and detecting searches.

## Synergy Classes

Classes with strong synergies: Smuggler, Thief, Caravaner

**Synergy Benefits**: 2x XP acquisition, enhanced effectiveness, reduced costs (scales with class level)

## Mechanics

| Effect    | Description                                           |
| --------- | ----------------------------------------------------- |
| Unlock    | Know when searches are coming (brief warning)         |
| Hiding    | +30% effectiveness hiding items on person or in cargo |
| Detection | Sense hidden compartments in others' cargo            |

**No scaling—fixed bonuses**

## XP Sources

- Smuggle goods: 10-20 XP
- Avoid searches: 15-25 XP
- Find hidden items: 5-15 XP
- Guided training: 10-100 XP/session
- Unguided training: 5-50 XP/session

## Level-Up Availability

- Strong synergy: ~500 XP (~50 actions)
- Moderate synergy: ~1000 XP
- No synergy: ~1500 XP

## Related Content

- **Strong Synergy:** [Smuggler](../../classes/rogue/smuggler/), [Thief](../../classes/rogue/thief/), [Caravaner](../../classes/trader/caravaner/)
- **See Also:** [Skills Index](../index.md)

## Synergy Examples

### Strong (2x XP)

**Smuggler Level 15** - Contraband specialist

- +40% hiding
- Enhanced warning
- 2x XP from smuggling

### Moderate (1.5x XP)

**Thief Level 10** - Concealment expert

- +35% hiding

### None (1x XP)

**Knight** - No connection

- No bonuses
