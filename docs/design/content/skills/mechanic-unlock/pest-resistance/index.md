---
title: Pest Resistance
type: mechanic-unlock
tier-type: Mechanic Unlock
summary: Protect crops and animals from pests
tags: [Agriculture]
---

# Pest Resistance

## Overview

**Type**: Mechanic Unlock
**Pool**: Agriculture
**Description**: Protect crops and animals from pests with early detection and treatment knowledge.

## Synergy Classes

Classes with strong synergies: Farmer, Rancher, Beekeeper

**Synergy Benefits**: 2x XP acquisition, enhanced effectiveness, reduced costs (scales with class level)

## Mechanics

| Effect        | Description                                    |
| ------------- | ---------------------------------------------- |
| Unlock        | -50% pest/disease occurrence on your farm      |
| Early Warning | Detect infestations before they spread         |
| Treatment     | Know remedies for common agricultural diseases |

**No scaling—binary unlock**

## XP Sources

- Deal with infestations: 15-25 XP
- Protect harvests: 10-20 XP
- Guided training: 10-100 XP/session
- Unguided training: 5-50 XP/session

## Level-Up Availability

- Strong synergy: ~500 XP (~50 actions)
- Moderate synergy: ~1000 XP
- No synergy: ~1500 XP

## Related Content

- **Strong Synergy:** [Farmer](../../classes/gatherer/farmer/), [Rancher](../../classes/rancher/), [Beekeeper](../../classes/beekeeper/)
- **See Also:** [Skills Index](../index.md)

## Synergy Examples

### Strong (2x XP)

**Farmer Level 15** - Pest management specialist

- -65% pest occurrence
- 2x XP from pest management

### Moderate (1.5x XP)

**Beekeeper Level 10** - Colony protection

- -55% pest occurrence

### None (1x XP)

**Scout** - No connection

- No bonuses
