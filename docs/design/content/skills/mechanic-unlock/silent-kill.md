---
title: Silent Kill
type: mechanic-unlock
tier-type: Mechanic Unlock
summary: Eliminate without alerting others
tags: [Stealth/Assassination, Combat/Assassination, Criminal/Assassination]
---

# Silent Kill

## Overview

**Type**: Mechanic Unlock
**Pool**: Stealth
**Description**: Eliminate targets without alerting nearby enemies.

## Synergy Classes

Classes with strong synergies: Assassin

**Synergy Benefits**: 2x XP acquisition, enhanced effectiveness, reduced costs (scales with class level)

## Mechanics

| Effect     | Description                                                 |
| ---------- | ----------------------------------------------------------- |
| Unlock     | Stealth kills don't alert nearby enemies (within reason)    |
| Body       | Target dies silently, body can be hidden instantly          |
| Limitation | Doesn't work if target in conversation or directly observed |

**No scaling—binary unlock**

## XP Sources

- Kill without detection: 20-35 XP
- Remain unnoticed after kills: 15-25 XP
- Guided training: 10-100 XP/session
- Unguided training: 5-50 XP/session

## Level-Up Availability

- Strong synergy: ~500 XP (~50 actions)
- Moderate synergy: ~1000 XP
- No synergy: ~1500 XP

## Related Content

- **Strong Synergy:** [Assassin](../../classes/rogue/assassin/)
- **See Also:** [Skills Index](../index.md)

## Synergy Examples

### Strong (2x XP)

**Assassin Level 15** - Silent death specialist

- Extended silence radius
- 2x XP from silent kills

### Moderate (1.5x XP)

**Thief Level 10** - Stealth expert

- Better concealment

### None (1x XP)

**Warrior** - No connection

- No bonuses
